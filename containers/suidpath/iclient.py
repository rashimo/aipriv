...
def recv_message(sock, timeout=None):
    """Receive a length-prefixed message. Returns None on timeout, raises on disconnect."""
    try:
        ...
        return None  # Timeout is not an error, just no data yet
...
def start_client(host="127.0.0.1", port=65432):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client_socket:
        ...
        while True:
            try:
                raw_data = recv_message(client_socket, timeout=60.0)
                if raw_data is None:
                    # Timeout - check if connection is still alive
                    try:
                        client_socket.send(b'')  # Test socket
